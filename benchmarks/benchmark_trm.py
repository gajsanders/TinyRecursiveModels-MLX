...
def benchmark_trm():
    ...
    print(f"Model parameters: input_dim={input_dim}, latent_dim={latent_dim}, output_dim={output_dim}")
    
    # Create sample input data
    batch_size = 32
    x = mx.array(np.random.rand(batch_size, input_dim).astype(np.float32))
    y = mx.array(np.random.rand(batch_size, output_dim).astype(np.float32))  # For recursive reasoning
    ...
    print(f"Recursive reasoning ({num_steps} steps) completed in {recursive_time:.4f} seconds")
    ...
    print(f"After model init: {memory_after_init:.2f} (Δ: {memory_after_init - initial_memory:.2f})")
    print(f"After forward: {memory_after_forward:.2f} (Δ: {memory_after_forward - memory_after_init:.2f})")
    print(f"After recursive: {memory_after_recursive:.2f} (Δ: {memory_after_recursive - memory_after_forward:.2f})")
